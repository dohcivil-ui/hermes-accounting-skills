...
- ส่ง `context.project` กลับตามผล query ซึ่งใช้รหัสโครงการเมื่อมี ID; ห้ามแปลงกลับเป็นชื่อก่อนส่ง ชื่อสำหรับแสดงผลใช้จาก `result` โดย context เก่าที่เป็นชื่อยังรองรับ
- สำหรับ `totals`, `transaction_count` (รวมถึงใน `projects`) และ `grounding.matched_transaction_count` นับรวมรายรับและรายจ่ายตามช่วงเวลา/โครงการ/บุคคล แม้ `metric` เป็น `expense` หรือ `income` ให้เรียกว่า "รายการรวมรายรับและรายจ่าย" ห้ามใช้ตอบจำนวนรายจ่ายหรือรายรับอย่างเดียว; กติกานี้ไม่เปลี่ยนจำนวนใน `party_summary` ที่กรองตาม metric แล้ว
...
- ตอบตัวเลขได้เมื่อคำสั่ง query สำเร็จและได้ JSON `ok: true` พร้อม `result` และ `grounding` จากการรันครั้งนั้นเท่านั้น; หาก exit code ไม่เป็นศูนย์ มี traceback หรือไม่มีผลสำเร็จ ให้หยุดสรุปและแจ้งว่ายังคำนวณไม่สำเร็จ ห้ามตอบยอดทดแทนรวมถึงศูนย์หรืออ้างว่าสรุปสำเร็จ
- เมื่อ query ล้มเหลว ห้ามอ่านชีตหรือรวมยอดผ่านเส้นทางอื่นเพื่อทดแทน ห้ามข้ามแถวที่ผิดและห้ามใช้ `created_at` หรือวันที่อื่นแทน `date`
- แจ้งสาเหตุเท่าที่ผลผิดพลาดระบุ เช่น วันที่รายการไม่ครบ; หากไม่มีผลรันให้บอกว่ายังตรวจสอบไม่ได้ ห้ามสร้างผลรันขึ้นเองหรือแก้ข้อมูลเพื่อให้ query ผ่าน
